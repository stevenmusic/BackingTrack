...
for (const cs of cfg.cases) {
  ...
  await pg.evaluate(() => { window.__sm = { hit: 0, miss: 0 }; const o = window.playSMSample;
    window.playSMSample = function () { const r = o.apply(this, arguments); r ? __sm.hit++ : __sm.miss++; return r; }; });
  ...
  const dd = await pg.evaluate(() => { const r = __d.red.slice().sort((a, b) => a - b);
    return { satIn_pk: +__d.pk.toFixed(3), hardclip_pct: +(100 * __d.over8 / __d.n).toFixed(3), over06_pct: +(100 * __d.over6 / __d.n).toFixed(2),
             ...
             SM: __sm, TLred_max_dB: limRed.length ? +(20*Math.log10(Math.min(...limRed))).toFixed(2) : null }; });
