...
- **公開網站給其他人用,暫時不做 App**
- 目標音質是 **Suno 等級**:取樣加 MIDI 的引擎天花板在 BIAB 等級,所以高品質版本改走神經網路生成(ACE-Step,MIT 授權),
  現在的引擎負責和弦與拍子的正確性、離線預覽與導引音軌
- **每次生成不能有額外費用**,前置費用(自備 GPU 機器)可以。不接按次計費的 API
