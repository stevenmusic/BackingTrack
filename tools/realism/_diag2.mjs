...
for (const cs of cfg.cases) {
  ...
  await pg.evaluate(() => { window.__g = { smp: 0, ks: 0, miss: {} }; const o = window.gtrSample;
    window.gtrSample = function (m, t) { const r = o(m, t); if (r) __g.smp++; else { __g.ks++; __g.miss[t + ':' + m] = 1; } return r; }; });
  ...
  const dd = await pg.evaluate(() => { const r = __d.red.slice().sort((a, b) => a - b);
    return { satIn_pk: +__d.pk.toFixed(3), hardclip_pct: +(100 * __d.over8 / __d.n).toFixed(3), over06_pct: +(100 * __d.over6 / __d.n).toFixed(2),
             ...
             GTR: __g, TLred_max_dB: limRed.length ? +(20*Math.log10(Math.min(...limRed))).toFixed(2) : null }; });
